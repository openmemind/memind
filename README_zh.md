...
如果你希望把 Memind 作为 Java 进程内的记忆引擎使用，而不是调用独立部署的 `memind-server`，可以使用这种方式。

| 运行方式 | 适合场景 |
|----------|----------|
| Spring Boot starter | 在 Boot 应用中使用 Spring 配置和自动配置的 AI/JDBC beans |
| 纯 Java | 希望完全控制 `Memory.builder()`、模型客户端、存储、向量检索和运行时选项 |

#### 添加依赖

先导入 Memind BOM，再引入 core runtime、Spring AI 插件以及一个 JDBC 方言插件。

下面的依赖片段展示的是纯 Java 接入方式。如果使用 Spring Boot，请使用
`memind-plugin-ai-spring-ai-starter`、`memind-plugin-jdbc-starter` 和 Spring 配置。
...
如果使用 MySQL 或 PostgreSQL，把 `memind-plugin-jdbc-sqlite` 替换为
`memind-plugin-jdbc-mysql` 或 `memind-plugin-jdbc-postgresql`。

#### 最小使用方式

`Memory` runtime 组装完成后，核心 API 很小：
...
如果你想看一个配置集中、默认值清晰的纯 Java 运行时组装方式，可以从
...
| 示例 | 展示内容 |
|------|----------|
| `quickstart` | 基础 `addMessages` 和检索流程 |
| `agent` | Agent 作用域记忆、可复用任务经验和 Agent 检索 |
| `insight` | 多批次提取与 Insight Tree 检索 |
| `document` | Document rawdata 摄取与可检索文档记忆 |
| `foresight` | Foresight 提取与面向未来的上下文检索 |
| `tool` | 工具调用上报、工具记忆和工具统计 |
...
Memind 可以通过官方 Agent 插件、SDK、REST API 或内置 HTTP MCP Server 接入。
你可以根据 AI 系统运行的位置选择合适的集成方式。

<a id="agent-integrations"></a>

### 将 Memind 接入任意 Agent

如果你希望 Memind 自动召回上下文、注入记忆，并跨会话捕获 Agent 活动，优先使用官方 Agent 集成。

Memind 为主流 Agent 提供官方集成：

- [`Claude Code`](./memind-integrations/claude-code)：项目长期记忆、SessionStart 连续性上下文、tool-aware 上下文注入，以及 coding-agent timeline 自动写入。
- [`Codex`](./memind-integrations/codex)：项目长期记忆、prompt/tool 上下文注入、带重试的 timeline 写入，以及 Codex 会话 source tagging。
- [`OpenClaw`](./memind-integrations/openclaw)：prompt 前自动召回 Memind 记忆，并把完成后的 OpenClaw agent 活动写成 `agent_timeline` raw data。
- [`Hermes`](./memind-integrations/hermes)：原生 Hermes memory provider，在每轮前召回相关上下文，并在响应后捕获完成的 Hermes 活动。

<a id="official-api-clients"></a>

### SDK 与 REST API

如果你要把 Memind 接入应用、后端服务、chatbot、工作流系统或自定义 Agent，可以使用 REST API 或官方 SDK。
...
当你的系统已经有自己的应用进程，并希望把 Memind 作为独立 memory service 使用时，优先选择 REST API 或 SDK。

### Open API 写入语义

默认写入端点（`/open/v1/memory/extract`、`/open/v1/memory/add-message` 和 `/open/v1/memory/commit`）
是 fire-and-forget：HTTP 成功响应表示 Memind 已接受并派发任务，不表示 extraction 已完成。
需要重试语义的客户端应该使用 `/open/v1/memory/sync/extract`，传入调用方自己持有的 raw-content payload，
并且只在返回的 extraction status 为 `SUCCESS` 时清除本地重试状态。

`/open/v1/memory/sync/add-message` 和 `/open/v1/memory/sync/commit` 会返回即时的 server-buffer 成功或失败，
但它们不是 durable replay boundary，因为 buffered conversation state 由 server 持有。

<a id="mcp-server"></a>

### HTTP MCP Server

如果你的 Agent 支持 Model Context Protocol，可以通过 MCP 将 Memind 作为工具服务接入。

`memind-server` 内置了一个无状态 HTTP MCP server，默认路径为 `/mcp`，默认启用。
它会把 Memind 记忆能力暴露给兼容 MCP 的 Agent，并复用同一套运行时、数据库、配置和日志。
