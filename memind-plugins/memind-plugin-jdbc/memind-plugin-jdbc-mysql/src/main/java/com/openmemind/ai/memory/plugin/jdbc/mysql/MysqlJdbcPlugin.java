...
public final class MysqlJdbcPlugin {

    private MysqlJdbcPlugin() {}

    public static JdbcMemoryAccess create(String jdbcUrl, String username, String password) {
        return create(
    ...
    public static JdbcMemoryAccess create(DataSource dataSource, JdbcPluginOptions options) {
        ...
        var insightBufferStore = new MysqlInsightBuffer(dataSource, options.createIfNotExist());
        var conversationBufferAccessor =
                new MysqlConversationBufferAccessor(dataSource, options.createIfNotExist());
        var bubbleTrackerStore =
                new MysqlBubbleTrackerStore(dataSource, options.createIfNotExist());
        var memoryBuffer =
                MemoryBuffer.of(
                        insightBufferStore,
        ...
        return new DefaultJdbcMemoryAccess(
                store,
                memoryBuffer,
                new MysqlMemoryTextSearch(dataSource, options.createIfNotExist()),
                bubbleTrackerStore,
                dataSource,
