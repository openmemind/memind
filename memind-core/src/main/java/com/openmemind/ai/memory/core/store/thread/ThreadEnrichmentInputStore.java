...
/**
 * Authoritative append-only store for replay-safe thread enrichment inputs.
 */
public interface ThreadEnrichmentInputStore {

    ThreadEnrichmentAppendResult appendRunAndEnqueueReplay(
            MemoryId memoryId,
            long replayCutoffItemId,
            List<MemoryThreadEnrichmentInput> runInputs);

    List<MemoryThreadEnrichmentInput> listReplayable(
