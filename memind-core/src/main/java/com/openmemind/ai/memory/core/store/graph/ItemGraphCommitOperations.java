...
/**
 * Store-owned coordinator for commit-critical item+graph batches.
 */
public interface ItemGraphCommitOperations {

    ItemGraphCommitReceipt commit(
            MemoryId memoryId,
            ExtractionBatchId extractionBatchId,
            List<MemoryItem> items,
            ItemGraphWritePlan writePlan);

    Optional<ExtractionBatchRecord> getBatch(
            MemoryId memoryId, ExtractionBatchId extractionBatchId);

    default void retryFailedBatchPromotion(MemoryId memoryId, ExtractionBatchId extractionBatchId) {
        throw new UnsupportedOperationException("retry promotion not supported");
    }

    default void discardFailedBatch(MemoryId memoryId, ExtractionBatchId extractionBatchId) {}
}
