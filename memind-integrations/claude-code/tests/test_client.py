...
class _FakeSyncMemory:
    ...
    def query_items(self, request):
        ...
        return SimpleNamespace(items=[SimpleNamespace(id="it-1", text="Use mvn test")])

    def query_raw_data(self, request):
        self.calls.append(("query_raw_data", request))
        return SimpleNamespace(raw_data=[SimpleNamespace(id="rd-1", caption="Fixed retry replay")])
...
class ClientTest(unittest.IsolatedAsyncioTestCase):
    ...
    def test_query_wrappers_use_official_sync_query_models(self):
        ...
        self.assertEqual(len(_FakeSyncMemindClient.instances), 2)
        ...
        self.assertEqual(item_request.categories, ["directive"])
        self.assertEqual(item_request.metadata_filter.all[0].path, "projectSlug")
